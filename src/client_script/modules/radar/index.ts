...
mp.radar = {
    getAnchor: () => {
        ...
        let minimap = {
            width: bigmap.status == 2 ? scaleX * (resolution.x / (2.45 * aspectRatio)) : scaleX * (resolution.x / (4 * aspectRatio)),
            height: bigmap.status == 2 ? scaleY * (resolution.y / 2.4) : scaleY * (resolution.y / 5.674),
            scaleX: scaleX,
            scaleY: scaleY,
